...
@Component({
  selector: 'app-listar-cabeceras-inventario',
  templateUrl: './listar-cabeceras-inventario.component.html',
  styleUrls: ['./listar-cabeceras-inventario.component.css'],
  //inyectar pipes
  providers:[TitleCasePipe, DecimalPipe]
})
export class ListarCabecerasInventarioComponent {
  ...
  verCalculos(idCabecera:number){
    this.router.navigateByUrl(`/administracion/listar-calculosRendicion/${idCabecera}`);
  }

  verDetalle(idCabecera: number) {
    this.router.navigateByUrl(`/administracion/listar-Rendicion/${idCabecera}`);
  }
